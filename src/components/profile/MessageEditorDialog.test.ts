...
describe('MessageEditorDialog', () => {
  it('loads the current message values and focuses the textarea', async () => {
    ...
    expect(wrapper.get('.message-editor__panel').attributes('role')).toBe('dialog')
    expect(wrapper.get('.message-editor__actions').text()).toContain('Rimuovi')
    expect(wrapper.get('.message-editor__actions').text()).toContain('Salva')
    wrapper.unmount()
    trigger.remove()
  })
